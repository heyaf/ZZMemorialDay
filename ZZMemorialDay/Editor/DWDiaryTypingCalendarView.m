...
- (UILabel *)labelNong {
    if (!_labelNong) {
        ...
        _labelNong.font = [UIFont fontWithName:@"STHeitiSC-Light" size:15];
        _labelNong.textColor = [UIColor whiteColor];
    }
    return _labelNong;
}
- (UILabel *)labelSmall {
...
- (void)setDicDate:(NSDictionary *)dicDate {
    if (dicDate) {
        ...
        rect.origin.x = self.frame.size.width / 2 - rect.size.width / 2;
        rect.origin.y = CGRectGetMaxY(self.labelWeek.frame);
        self.labelNong.frame = rect;
        
//        [self.chooseButton setTitle:_dataArray[0] forState:0];
