...
@property (nonatomic, strong)  DropDownListPicker *picker;
...
- (void)viewDidLoad {
    ...
    UIImageView *bgImageV = [[UIImageView alloc] initWithFrame:CGRectMake(0, DWDiaryNavigationBarHeight, rect.size.width, rect.size.height - CGRectGetHeight(_navigationBar.frame))];
...
//    down.listArray = @[@"全部分类",@"小日常",@"工作",@"生日"];
//    down.frame = CGRectMake(15, CGRectGetMaxY(_segment.frame) / 2 + DWDiaryNavigationBarHeight / 2 - 12, 100, 40);
//    [self.view addSubview:down];
//    down.delegate = self;
//    self.down = down;
    
    UIButton *btn = [UIButton buttonWithType:0];
    btn.frame = CGRectMake(5, CGRectGetMaxY(_segment.frame) / 2 + DWDiaryNavigationBarHeight / 2 - 10, 90, 40);
    [btn setTitle:@"全部分类" forState:0];
    btn.titleLabel.font = kFont(14);
    [btn setImage:[UIImage imageNamed:@"下拉"] forState:0];
    [btn setImagePositionWithType:SSImagePositionTypeLeft spacing:5];
    [btn setTitleColor:[UIColor blackColor] forState:0];
    [btn addTarget:self action:@selector(clieckChoose:) forControlEvents:UIControlEventTouchUpInside];
    [_navigationBar addSubview:btn];
    _pickbtn = btn;
    NSArray *nameArr = @[@"全部分类",@"小日常",@"工作",@"生日"];
    DropDownListPicker *picker = [[DropDownListPicker alloc] initWithOrigin:CGPointMake(0, CGRectGetHeight(_navigationBar.frame)) title:@"选择分类" itemTitles:nameArr selectedTitle:@"全部分类" selection:^(NSInteger selectedIndex) {
        if (selectedIndex>=0&&selectedIndex<=3) {
            [btn setTitle:nameArr[selectedIndex] forState:0];
            self.typeSelect = selectedIndex;
            [self updateTableViewDataSourceFromDataBase];
        }
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    //注册通知,监听键盘消失事件
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(setBGimage)
...
-(void)setBGimage{
    ...
    if ([str isEqualToString:@"1"]) {
        UIImage *getimage2 = [[SDImageCache sharedImageCache] imageFromCacheForKey:@"/Documents/pic.png"];        _bgImageV.image = getimage2;
    }
}
...
- (void)updateTableViewDataSourceFromDataBase {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        for (int i = 0; i < objects.count; i++) {
            ...
            }else if ([[objects[i] valueForKey:@"type"] integerValue] ==self.typeSelect) {
                [tmpArray addObject:[objects objectAtIndex:i]];
            }
        }
        // sort
...
- (void)updateTypingViewContentFromDataBaseWithDicDate:(NSDictionary *)dicDate {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        for (int i = 0; i < objects.count; i++) {
            ...
            _typingView.type = [[objects objectAtIndex:i] valueForKey:@"type"];

        }
    });
}

- (void)saveModelIntoDataBaseWithTitle:(NSString *)title detail:(NSString *)detail emotion:(NSString *)emotion type:(NSString *)type weather:(NSString *)weather dicDate:(NSDictionary *)dicDate{
    ...
    dispatch_group_async(dispatchGroup, dispatch_get_main_queue(), ^{
        ...
        [theDiary setValue:type forKey:@"type"];
...
-(void)saveModelWithTitle:(NSString *)title detail:(NSString *)detail emotion:(NSString *)emotion weather:(NSString *)weather type:(NSString *)type{
    [self saveModelIntoDataBaseWithTitle:title detail:detail emotion:emotion type:type weather:weather  dicDate:_pickerView.dicDate];
...
-(void)clieckChoose:(UIButton *)sender{
